...
- **Shared packages**: Four tiers live under `packages/`, each with a distinct scope (see [ADR-0004](adr/0004-layered-package-architecture.md)):
    ...
    - `@packages/api` — The **API layer**. Two internal layers: (1) **Entities** — plain TS interfaces and date parsing (`./entities/plants`, `./entities/auth`, `./entities/household`, `./entities/responsibility`, `./entities/care-events`); (2) **Handlers** — MSW runtime + storybook factory handlers (`./handlers/*`). DB singletons (`plantsDb`, `householdDb`, `assignmentDb`, `careEventDb`) and seed data are internal — never exported. Handlers are self-contained and do not import from module feature code. TanStack Query hooks live in modules, not here — the API package has zero React dependency.
...
- **Handlers** — MSW request handlers live in `@packages/api/handlers/<module-name>/`, scoped to `/api/<prefix>/<entity>` URLs (e.g., `/api/management/plants`, `/api/today/plants`, `/api/management/user/profile`, `/api/household/*`, `/api/today/assignments`, `/api/today/care-events`). Modules import handlers from `@packages/api` for registration and Storybook setup.
- **Shared DB** — All modules read/write the same in-memory stores inside `@packages/api`. DB singletons (`plantsDb`, `householdDb`, `assignmentDb`, `careEventDb`) are internal to the api package — never exported. Cross-module visibility works through the shared DB (e.g., both management and watering modules access household data via their own MSW handlers reading from `householdDb`). The host seeds all DBs via `seedDatabase()` from `@packages/api/seed`.
