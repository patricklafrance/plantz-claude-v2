...
- **Shared packages**: Three tiers live under `packages/`, each with a distinct scope:
    ...
    - `@packages/core-plants` — Cross-module **plant** code: types, schemas, DB singletons, collection factories, shared UI components, and test utilities. Also houses household types, schemas, and DB (`./household` subpath). Subpath exports: `./collection`, `./db`, `./care-event` (care event types and schema only), `./household` (household types and schema), `./test-utils`. Module-specific logic (vacation planning, adjustment recommendations, care insights computation) lives in the consuming module (`@modules/watering`).
...
- **Handlers** — MSW request handlers live in the module's `mocks/` folder, scoped to `/api/<prefix>/<entity>` URLs (e.g., `/api/management/plants`, `/api/today/plants`, `/api/management/household`, `/api/today/assignments`). Every module must own the MSW handlers for the endpoints it uses — never rely on the host or another module for handlers.
- **Shared DB** — All modules read/write the same in-memory stores, exposed via `@packages/core-plants/db`: `plantsDb` (plants), `careEventsDb` (care events), and `householdDb` (households and members). Cross-module visibility works through the shared DB, not shared client-side collections. Modules may also own **module-local** in-memory DBs for entities that only one module consumes (e.g., `adjustmentsDb` in the watering module's today subfolder, `vacationDb` in the watering module's vacation-planner subfolder, `assignmentsDb` for responsibility assignments in the watering module's today subfolder). These are not shared — promote to a shared package only when a second module needs the same data.
