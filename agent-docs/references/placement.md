...
7. **Daily watering tasks, responsibility assignments, or care tracking?** → `@modules/watering`
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `@modules/management` | Plant identity, ownership, configuration, user account/preferences, and household coordination (create/join households, invite members).                                  | `inventory/`, `account/`, `household/` |
| `@modules/watering`   | Daily care execution — what needs the user's attention now (plants due for watering), responsibility assignments, care event tracking, and duplicate watering prevention. | `today/`                               |
...
- **management/inventory/** — Plant CRUD, plant details, plant list views, shared plant indicators
- **management/account/** — User profile, preferences, settings
- **management/household/** — Household CRUD, member list, invite flow
- **watering/today/** — Daily care dashboard, watering actions, responsibility assignments, care event tracking, duplicate watering prevention
...
| `@packages/api`         | API layer: entity types and date parsing (`./entities/plants`, `./entities/auth`, `./entities/household`, `./entities/responsibility`, `./entities/care-events`), MSW handlers (`./handlers/*`), test factories (`./test-utils`). DB singletons (`plantsDb`, `householdDb`, `assignmentDb`, `careEventDb`) and seed data are internal. Zero React dependency. | No routing. No React hooks. Handlers do not import from module feature code. |
