...
describe("Progress", () => {
    ...
    describe("start", () => {
        it("should not log immediately when verbose is false", () => {
            const progress = new Progress({ verbose: false });
            progress.start("plan", "Analyzing codebase");

            expect(logSpy).not.toHaveBeenCalled();
        });

        it("should log immediately when verbose is true", () => {
            const progress = new Progress({ verbose: true });
            progress.start("plan", "Analyzing codebase");
