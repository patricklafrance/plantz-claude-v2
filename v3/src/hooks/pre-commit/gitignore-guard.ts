...
export async function gitignoreGuard(cwd: string): Promise<string[]> {
    ...
    if (violations.length > 0) {
        return [
            `[gitignore-guard] .gitignore must not un-ignore .adlc/ or .adlc-metrics/ paths (all ADLC artifacts are ephemeral):\n${violations.map(v => `  ${v}`).join("\n")}`
        ];
    }
