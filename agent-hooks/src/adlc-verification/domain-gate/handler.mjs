...
export default function handleDomainGate(cwd) {
    const problems = [...noPlanMutations(cwd)];

    if (hasFile(cwd, "domain-gate-revision.md")) {
        const content = readFileSync(resolve(cwd, ".adlc", "domain-gate-revision.md"), "utf8");
        if (!ISSUE_RE.test(content)) {
            problems.push(
                "Revision lacks issues: `.adlc/domain-gate-revision.md` exists but contains no `### ISSUE-N` blocks. " +
