...
function recentTestFingerprints(recentEvents) {
    return recentEvents
...
function nudgeMessage(consecutiveCount, recentEvents) {
...
function escalationMessage(consecutiveCount, recentEvents) {
    const fingerprints = recentTestFingerprints(recentEvents);
...
function budgetMessage(totalCalls) {
    return [
...
/**
 * @param {object} event - normalized event with toolName, isTestCommand, etc.
 * @param {object} state - supervisor state with test.* fields
 * @returns {null | { action: string, severity: string, tier?: number, reason: string }}
 */
export default function checkTestThrash(event, state) {
