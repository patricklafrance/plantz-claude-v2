...
2. Spawn `subagent_type: "_adlc-coder"` with the slice file and `mode: draft`.
3. Spawn `subagent_type: "_adlc-reviewer"` pointing at the slice file.
...
6. Resume the coder via `SendMessage` with `mode: revision` and the saved verification report as `verification-results`.
