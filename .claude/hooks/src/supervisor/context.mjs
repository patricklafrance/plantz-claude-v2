...
import { clearRecovery, readRecovery, writeRecovery } from "./recovery.mjs";
...
export function createPreToolContext(toolName, toolInput, cwd, input = {}) {
    ...
    const recovery = readRecovery(cwd);
...
export function persistContext(context, state = context.nextState, recovery = context.recovery) {
    writeState(context.cwd, state);
    appendEvent(context.cwd, context.event);

    if (recovery?.active) {
        writeRecovery(context.cwd, recovery);
    } else {
        clearRecovery(context.cwd);
    }
}
